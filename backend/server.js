...
// In-memory storage for health history and chat sessions
const healthHistory = [];
const chatSessions = new Map();

// Advanced health analysis algorithms
...
const calculateHydrationNeeds = (weight, exerciseMinutes) => {
  // Base: 35ml per kg, plus extra for exercise
  let baseWater = weight * 35;
  baseWater += exerciseMinutes * 12; // 12ml per minute of exercise
  return Math.round(baseWater / 250); // Convert to glasses (250ml each)
};
...
const analyzeHealth = (data) => {
  const { 
    ...
    stressLevel: userStressLevel = 5, bloodPressureSystolic, bloodPressureDiastolic
  } = data;
  ...
  const restingHRPercentage = (heartRate / maxHeartRate) * 100;
  ...
  if (heartRateScore < 100) {
    recommendations.push({
      ...
      priority: heartRate > 100 ? 'high' : 'medium',
  ...
  if (exerciseScore < 85) {
    recommendations.push({
      category: 'Fitness',
      priority: exerciseScore < 50 ? 'high' : 'medium',
  ...
  if (waterScore < 85) {
    recommendations.push({
      category: 'Hydration',
      priority: waterScore < 50 ? 'high' : 'low',
  ...
  if (stressAnalysis.score > 40) {
    recommendations.push({
      ...
      priority: stressAnalysis.score > 60 ? 'high' : 'medium',
  ...
  return {
    ...
    metrics: {
      ...
      heartRate: { 
        ...
        zone: heartRateZone,
        maxHR: maxHeartRate,
        targetZone: `${Math.round(maxHeartRate * 0.5)}-${Math.round(maxHeartRate * 0.85)}`
      },
      bloodPressure: {
        systolic: bloodPressureSystolic || null,
        diastolic: bloodPressureDiastolic || null,
        category: bpCategory,
        score: bpScore,
        risk: bpRisk
      },
      ...
      exercise: { 
        ...
        weeklyTotal: weeklyExercise,
        status: exerciseStatus, 
        score: exerciseScore,
        level: exerciseLevel,
        recommended: '150-300 min/week'
      },
      water: { 
        value: waterIntake, 
    ...
    advancedMetrics: {
      bmr,
      tdee,
      idealWeight: { min: idealWeight - 5, max: idealWeight + 5, target: idealWeight },
      bodyFatEstimate,
      metabolicAge,
      caloriesForWeightLoss: tdee - 500,
      caloriesForWeightGain: tdee + 500
    },
    recommendations,
    urgentAlerts,
...
app.get('/api/health', (req, res) => {
  res.json({ 
    message: 'Health Analyzer API v2.0', 
    status: 'healthy',
...
// Chat endpoint for health AI assistant
app.post('/api/chat', async (req, res) => {
  try {
    ...
    res.status(500).json({ 
      error: 'Failed to get response from health assistant',
  ...
  try {
    ...
    const chatCompletion = await groq.chat.completions.create({
      messages: [
        { 
          role: 'system', 
          content: 'You are a health expert. Provide one short, actionable health tip. Be concise (1-2 sentences max). Include one relevant emoji at the start.' 
        },
        { 
          role: 'user', 
          content: `Give me a unique, practical ${randomCategory} tip for better health.` 
        }
      ],
      model: 'llama-3.3-70b-versatile',
      temperature: 0.9,
